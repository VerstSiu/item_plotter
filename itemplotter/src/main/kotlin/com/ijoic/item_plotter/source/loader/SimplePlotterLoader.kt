...
/**
 * Simple plotter loader.
 *
 * <p>Returns cache or first create plotter instance.</p>
 *
 * @author xiao.yl on 2018/1/25.
 * @version 1.0
 */
class SimplePlotterLoader(
    private val loader: (Context) -> Plotter): PlotterLoader {

  private val lock = Object()
  private var cachePlotter: Plotter? = null

  override fun getPlotter(context: Context): Plotter {
    synchronized(lock) {
      val oldPlotter = cachePlotter
      val plotter = oldPlotter ?: loader.invoke(context)

      if (oldPlotter == null) {
        cachePlotter = plotter
      }
      return plotter
    }
  }
